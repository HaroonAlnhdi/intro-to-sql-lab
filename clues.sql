...
SELECT code,name, population, region FROM countries
WHERE region='Southern Europe'
ORDER BY population ASC
LIMIT 1; -- to select only the least populated country in Southern Europe


-- Clue #2: Now that we're here, 
--we have insight that Carmen was seen attending language classes in this country's officially recognized language. 
...
SELECT cl.language, c.name, c.region FROM countrylanguages cl
JOIN countries c
...
SELECT ci.name AS city_name,continent, ci.population, ci.district, ci.countrycode,co.name AS country_name, co.region
FROM cities ci , countries co
WHERE ci.countrycode = co.code AND ci.name LIKE 'Serra%' AND continent = 'South America'; 

-- the capital id is 211;
